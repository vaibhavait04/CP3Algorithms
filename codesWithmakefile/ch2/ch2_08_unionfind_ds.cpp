...
private:
  ...
  int numSets;
public:
  UnionFind(int N) { // 1. Initialize
    setSize.assign(N, 1); numSets = N; rank.assign(N, 0); // init rank as 0 for all 
    p.assign(N, 0); for (int i = 0; i < N; i++) p[i] = i; } // init - p is diff for each set

  int findSet(int i) { return (p[i] == i) ? i : (p[i] = findSet(p[i])); }  // 2. Found at i -> i else return find (p[i]) 
  bool isSameSet(int i, int j) { return findSet(i) == findSet(j); } // 3. Simple comparison 

  void unionSet(int i, int j) {  // 4. Union - check rank for short path - 
    if (!isSameSet(i, j)) { 
