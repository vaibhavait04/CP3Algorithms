...
int main (void) {
   ...
   while (getline(cin, input)) {
      ...
      string_to_bignum (a, &s1);
      string_to_bignum (b, &s2);

      if (compare_bignum (&s1, &i) == MINUS) { cout << "first number too big" << endl; }
      if (compare_bignum (&s2, &i) == MINUS) { cout << "second number too big" << endl; }
      if (c == '+')
         add_bignum (&s1, &s2, &result);
      else
         multiply_bignum (&s1, &s2, &result);

      if (compare_bignum (&result, &i) == MINUS) { cout << "result too big" << endl; }
   }
